...
AppLogic::AppLogic(
    ...
    const std::string &log_path) : m_server_ip(ip),
                                   m_server_port(port),
...
bool AppLogic::perform_authentication()
{
    ...
    if (!send_message({"LOGIN", m_user_name, -1, ""}))
        return false;
    ...
    if (!send_message({"CHALLENGE_RESPONSE", m_user_name, -1, response_hash}))
        return false;
...
void AppLogic::main_loop()
{
    while (is_connected())
    {
        ...
        default:
            m_console.show_error("Неверный выбор.");
            break;
        }
    }
}
...
{
    if (!send_message({"GET_PUB_KEY", m_user_name, -1, ""}))
        return false;
    ...
    {
        std::string error_msg = "Ошибка протокола при получении ключа. Получены заголовки: " + n_response->header + ", " + e_response->header;
        m_console.show_error(error_msg);
        m_logger->write_log(m_log_path, "[AppLogic] " + error_msg);
        return false;
    }
