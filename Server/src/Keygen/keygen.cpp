...
int main() {
    try {
        ...
        // 1. Генерируем новую пару ключей RSA
        RSAKeyPair key_pair(KEY_BIT_LENGTH);
        
        std::cout << "\nКлючевая пара успешно сгенерирована." << std::endl;
        key_pair.print(); // Показываем сгенерированные компоненты

        // 2. Сохраняем ключи в файлы
        std::cout << "\nСохранение ключей в файлы..." << std::endl;

        // Используем ваши готовые функции из модуля Rsa
        saveKeyToFile(PRIV_KEY_FILE, key_pair.d); // Приватный ключ (d)
        std::cout << " -> " << PRIV_KEY_FILE << std::endl;

        saveKeyToFile(PUB_N_FILE, key_pair.n);    // Модуль (n)
        std::cout << " -> " << PUB_N_FILE << std::endl;

        saveKeyToFile(PUB_E_FILE, key_pair.e);    // Публичная экспонента (e)
